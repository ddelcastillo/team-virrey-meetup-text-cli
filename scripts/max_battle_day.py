"""Max Battle Day text generation script with SQLite database integration.

This script generates Spanish text for Max Battle Day posts on Saturday or Sunday while using
...
from pokemon_meetup.utils.date_utils import get_max_battle_day_date
...
def generate_max_battle_day_text(
    *, pokemon_data: PokemonData, day_choice: int, max_type: str, is_shiny_available: bool
) -> str:
    """Generate the Max Battle Day text using the template system.
    ...
    Returns:
        Formatted Spanish text for Max Battle Day.
    """
    template_manager = get_template_manager()
    return template_manager.render_max_battle_day(
        pokemon_data=pokemon_data, day_choice=day_choice, max_type=max_type, is_shiny_available=is_shiny_available
    )
...
async def get_pokemon_input(*, service: "PokemonService") -> tuple[str, bool]:
    ...
    """
    while True:
        pokemon_name = input("\n🔍 Enter Pokémon name for Max Battle Day: ").strip()
...
def get_max_type_input() -> str:
    """Get Max type input from user.

    Returns:
        "Dynamax" for choice 1, "Gigantamax" for choice 2.
    """
    print("\n⚔️ Select Max type:")
    print("  1. Dynamax")
    print("  2. Gigantamax")
    ...
    while True:
        choice = input("Choose Max type (1/2): ").strip()
        ...
        if choice == "1":
            ...
            print("❌ Please enter '1' for Dynamax or '2' for Gigantamax.")
...
async def main() -> None:
    """Main function to run the Max Battle Day script."""
    print("⚔️ Max Battle Day Text Generator")
    print("=" * 50)
    ...
    try:
        ...
        # Get Max type from user
        max_type = get_max_type_input()

        # Show date information
        event_date = get_max_battle_day_date(day_choice=day_choice)
        print(f"📅 Event date: {event_date}")
        print(f"⚔️ Max type: {max_type}")
        ...
        print("🚫☁️ Max battles cannot be weather boosted")
        ...
        max_battle_text = generate_max_battle_day_text(
            pokemon_data=pokemon_data, day_choice=day_choice, max_type=max_type, is_shiny_available=is_shiny_available
        )

        print("\n" + "=" * 60)
        print("📝 GENERATED MAX BATTLE DAY TEXT:")
        print("=" * 60)
        print(max_battle_text)
        print("=" * 60)
        ...
        if copy_choice in ["y", "yes"]:
            try:
                ...
                pyperclip.copy(max_battle_text)
